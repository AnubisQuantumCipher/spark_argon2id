...
package body Spark_Argon2id.Zeroize is
   ...
   procedure Wipe_Salt (Buffer : in out Salt_Array) is
   begin
      --  Salt_Array is a subtype of Byte_Array, so we can iterate directly
      for I in Buffer'Range loop
         pragma Loop_Invariant (I in Buffer'Range);
         pragma Loop_Invariant (for all J in Buffer'First .. I - 1 =>
            Buffer (J) = 0);
   ...
   --  NOTE: Equal implements constant-time comparison using XOR accumulation.
