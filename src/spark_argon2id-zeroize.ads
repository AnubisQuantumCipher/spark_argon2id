...
package Spark_Argon2id.Zeroize is
   ...
   --  Wipe byte array (cryptographic zeroization)
   ...
   --  Wipe salt array (32 bytes)
   procedure Wipe_Salt (Buffer : in out Salt_Array)
     with
       Global  => null,
       Depends => (Buffer => Buffer),
       Post    => Is_Zeroed_Ghost (Byte_Array (Buffer));
